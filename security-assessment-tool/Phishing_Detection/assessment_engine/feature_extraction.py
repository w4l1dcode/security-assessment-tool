...
def generate_data_set(url):
    ...
    # 10.Favicon
    # TODO error hier als de link geen html pagina teruggeeft
    if soup == -999:
        data_set.append(-1)
    else:
        try:
    ...
    if soup == -999:
        ...
        try:
            ...
            if (percentage < 31.0) and (percentage != 0):
                data_set.append(1)
...
# def threaded_dataset_generator(i):
#     try:
#         extracted_features = generate_data_set(validphishes["url"][i])
#         extracted_features.append(-1)
#         dataset.append(extracted_features)
#         with open("phishingurl2.csv", "w") as my_csv:
    ...
    try:
        ...
        if len(extracted_features) != 30:
            ...
            with open("unable_to_check.csv", "w", newline='') as unable_csv:
                ...
                header = ['email-id', 'sender', 'subject', 'url', "result"]
            ...
            with open("Phishing_detection/assessment_engine/to_predict.csv", "w", newline='') as my_csv:
                csvwriter = csv.writer(my_csv, delimiter=',')
                header = ["having_IPhaving_IP_Address", "URLURL_Length", "Shortining_Service",
...
# to_predict = dataframe['url'].values
# Press the green button in the gutter to run the script.
# if __name__ == '__main__':
#     to_predict = pd.read_csv("to_predict.csv", header=0)
#
#     with concurrent.futures.ThreadPoolExecutor() as executor:
#         dataset = []
#         unable_to_check = []
